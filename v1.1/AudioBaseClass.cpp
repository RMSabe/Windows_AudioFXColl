...
VOID AudioBaseClass::setInputFileDirectory(__string file_dir)
{
	this->filein_dir = file_dir;
	return;
}

__string AudioBaseClass::getInputFileDirectory(VOID)
{
	return this->filein_dir;
}

VOID AudioBaseClass::setOutputFileDirectory(__string file_dir)
{
	this->fileout_dir = file_dir;
	return;
}

__string AudioBaseClass::getOutputFileDirectory(VOID)
{
...
__string AudioBaseClass::getLastErrorMessage(VOID)
{
	switch(this->status)
	{
...
BOOL AudioBaseClass::file_ext_check(const TCHAR *file_dir)
{
	...
	if(this->compare_signature(TEXT(".wav"), file_dir, (len - 4u))) return TRUE;
	if(this->compare_signature(TEXT(".WAV"), file_dir, (len - 4u))) return TRUE;
...
BOOL AudioBaseClass::compare_signature(const WCHAR *auth, const WCHAR *wordbuf, SIZE_T offset)
{
	if(auth == nullptr) return FALSE;
	if(wordbuf == nullptr) return FALSE;

	if(auth[0] != wordbuf[offset]) return FALSE;
	if(auth[1] != wordbuf[offset + 1u]) return FALSE;
	if(auth[2] != wordbuf[offset + 2u]) return FALSE;
	if(auth[3] != wordbuf[offset + 3u]) return FALSE;
